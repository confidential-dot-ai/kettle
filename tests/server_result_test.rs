...
use kettle::api::{BuildRequest, Event, BuildResult, JobIdResponse};
...
async fn result_endpoint_streams_tarball_when_done() {
    ...
    let resp = app
        .oneshot(Request::get(format!("/build/{job_id}/result"))
...
async fn result_endpoint_returns_404_when_unknown() {
    ...
    let resp = app
        .oneshot(Request::get("/build/missing/result").body(Body::empty()).unwrap())
        .await.unwrap();
...
#[derive(Clone)]
struct TestRunner;
impl TestRunner { fn new() -> Arc<Self> { Arc::new(Self) } }
    ...
    fn spawn(&self, registry: Arc<JobRegistry>, id: String, _req: BuildRequest) {
        tokio::spawn(async move {
            ...
            let mut enc = flate2::write::GzEncoder::new(
                f.as_file(), flate2::Compression::fast());
